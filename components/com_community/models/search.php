...
class CommunityModelSearch extends JCCModel
{
	...
	public function searchPeople($query , $avatarOnly = '', $friendId = 0 )
	{
		...
		if(isset($data['q']))
		{
			...
			else
			{
				...
				$filter[]	= 'UCASE(' . $nameType . ') LIKE UCASE(' . $db->Quote( '%' . $value . '%' ) . ')';
                                //redacron: we also need to search the user skills:
                                $filter_2[] = " (skillTags LIKE ".$db->Quote( '%' . $value . '%' ) ."  OR skill LIKE ".$db->Quote( '%' . $value . '%' ) ." OR skillDesc LIKE ".$db->Quote( '%' . $value . '%' ).")" ;
			}
		}
		...
		if(count($filter)> 0 || count($filterField > 0))
		{
            ...
            //SELECTIN is a redacron alteration. Add it to the regular query:
                                $SELECTIN = "b.id IN (SELECT userid FROM #__pf_project_skills_added WHERE ". implode(' OR ', $filter_2).") OR";
				$query .= ' WHERE '.$SELECTIN.' b.'.$db->quoteName('block').' = '.$db->Quote('0').' AND '.implode(' AND ',$filter). $filterquery;
