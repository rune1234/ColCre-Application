...
function getCategories()
    ...
    function createSelectCatg($f)
    {
        $freelancer_catg = getCategories();
        echo "<select name='field{$f->id}'>";
        foreach ($freelancer_catg as $fg)
        {
            $selected = ($f->value == $fg->id) ? 'selected="selected"' : '';
            echo "<option $selected value='$fg->id' >$fg->title</option>\n";
        }
        echo "</select>";
    }
						...
						?>
								<li>
									<label id="lblfield<?php echo $f->id;?>" for="field<?php echo $f->id;?>" class="form-label">
										<?php  echo " ";echo JText::_( $f->name );?>
                                                                                    <?php if($f->required == 1) echo '<span class="required-sign"> *</span>'; ?>
									</label>
									<div class="form-field">
										<?php 
                                                                                if ($f->fieldcode == 'SKILL_CATEGORY') createSelectCatg($f);
                                                                                
                                                                                else echo CProfileLibrary::getFieldHTML( $f , '' ); ?>
