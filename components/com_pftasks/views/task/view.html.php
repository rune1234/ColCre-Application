...
class PFtasksViewTask extends JViewLegacy
{
	...
	protected $user;
    protected $toolbar;
        function taskSkills($id)
        {
            if (!is_numeric($id)) return false;
            $db = JFactory::getDbo();
            $query = "SELECT a.task_id, a.project_id, b.skill FROM #__pf_project_skills as a INNER JOIN #__pf_skills as b ON b.id = a.skill_id WHERE a.task_id = $id LIMIT 100";
            $db->setQuery($query);
	...
	function display($tpl = null)
	{
		...
		$this->item	 = $this->get('Item');
                ...
                $this->skillsNeeded = $this->taskSkills($this->item->id);
