...
class projectMatches
{
       public function specifyMatch($description, $taskId, $projectId, $userId)
       {
             ...
             $skill = $this->getMatchDesc($userId, $db);
     ...
     public function getUserMatches($userid)
     {
         ...
         $query = "SELECT a.*, e.title as task_title, b.*,d.skill, c.title,c.alias,c.description, c.created_by, c.created, c.access, c.state, round((  (select count(*) "
        ...
        private function getMatchDesc($userId, & $db)
    {
        //$db = JFactory::getDbo();
         if (!is_numeric($userId) ) return;
