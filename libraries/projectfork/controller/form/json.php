 ...
 *
 */
class PFControllerFormJson extends JControllerForm
{
    /**
    ...
    private function _getUserMap($user_id, & $db) //redacron method
    {
       $query = "SELECT group_id FROM #__user_usergroup_map WHERE user_id = $user_id LIMIT 1";
       $db->setQuery($query);
       $level = $db->loadResult();
       if ($level > 5 && $level != 9) { return true;}//manager, administrator, and super_user
       else { return false;}
    }
    private function _projectInvited($project_id, $user_id, $owner_id, & $db)
    {
        //we don't check to see if project_id or user_id are numeric because the function is private
        if ($user_id == $owner_id) return true;
        $query = "SELECT * FROM #__pf_projects_invites WHERE project_id = $project_id AND invited = $user_id LIMIT 1";
        $db->setQuery($query);
        $row = $db->loadObject();
        return ($row) ? true : false;
    }
    public function comProjAccess($project_id, $user_id)//redacron function
    {
        if(!is_numeric($project_id) || !is_numeric($user_id)) return false;
        if ($user_id == 0) return false;
        $db = JFactory::getDbo();
        if ($this->_getUserMap($user_id, $db)) return true;//manager, administrators have access
        $query = "SELECT comm_perm, created_by FROM #__pf_projects WHERE id= $project_id LIMIT 1";
        $db->setQuery($query);
        $row = $db->loadObject();
        $created_by = $row->created_by;
        $comm_perm = $row->comm_perm;
        if ($comm_perm == 'registered') return true;
        elseif ($comm_perm == 'invited') { return $this->_projectInvited($project_id, $user_id, $created_by, $db); }
    }
    public function save_2()//redacron function used to allow users to enter comments on projects. It is a copy of save() with a few alterations
    {
        $rdata = array();
        ...
        if (!JSession::checkToken()) {
            ...
            //print_r($_POST['jform']['project_id']);
            //$_POST['jform']['project_id'];
          //   $_POST['jform']['item_id'];if it is a task, the item_id will be different from the project_id
        // Initialise variables.
        $app   = JFactory::getApplication();
        ...
        // Access check.
         if ($this->comProjAccess($_POST['jform']['project_id'], $user->id) === false)
        {
            $rdata['success']    = "false";
